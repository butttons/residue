...
pages.get("/:org", async (c) => {
  ...
  return c.html(
    <Layout title={`${org} — residue`}>
      <Breadcrumb items={[{ label: "residue", href: "/app" }, { label: org }]} />
...
pages.get("/:org/:repo/:sha", async (c) => {
  ...
  const sessionsData = await Promise.all(
    [...uniqueSessions.values()].map(async (session) => {
      ...
      let messages: { role: string; content: string; model?: string; tool_calls?: { name: string; input: string; output: string }[] }[] = [];
      try {
